...
// -------------------------------------------------------------------------------
// Raymarching - First Intersection Points
// -------------------------------------------------------------------------------

__global__ void ray_marching_intersection_kernel(
    // rays info
    const uint32_t n_rays,
    const float *rays_o, // shape (n_rays, 3)
    const float *rays_d, // shape (n_rays, 3)
...
std::vector<torch::Tensor> ray_marching_intersection(
    // rays
    const torch::Tensor rays_o,
    const torch::Tensor rays_d,
    const torch::Tensor t_min,
    ...
    const float cone_angle)
{
    ...
    // helper counter
    torch::Tensor intersection_indices = torch::full(
        {n_rays}, -1, rays_o.options().dtype(torch::kInt32));

    // count number of samples per ray
    ray_marching_intersection_kernel<<<blocks, threads, 0, at::cuda::getCurrentCUDAStream()>>>(
        // rays
        n_rays,
        rays_o.data_ptr<float>(),
        rays_d.data_ptr<float>(),
        ...
        nullptr, /* packed_info */
        // outputs
        intersection_indices.data_ptr<int>(),
        nullptr, /* ray_indices */
        nullptr, /* t_starts */
        nullptr /* t_ends */);
